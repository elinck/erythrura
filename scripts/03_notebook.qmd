...
Evidence of distinct clusters, with Solomons and PNG *trichroa* showing no geographic substructure. Another way to visualize this subtle differentiation is with a neighbor-joining tree. Here is one build from Nei's genetic distance (D), rooted at the midpoint, and ladderized:
...
im_df <- im %>% mutate(model = "IM", AIC = -2*ll_model + 2*6) %>% select(model, AIC, ll_model)
sc_df <- sc %>% mutate(model = "SC", AIC = -2*ll_model + 2*7) %>% select(model, AIC, ll_model)
si_df <- si %>% mutate(model = "SI", AIC = -2*ll_model + 2*4) %>% select(model, AIC, ll_model)
...
p8
```
The best run is an IM run by AIC and log-likelihood:
...
```{r, message=FALSE, warning=FALSE}
params <- read_tsv("/home/k14m234/erythrura/scripts/dadi/im_ci.tsv", col_names = FALSE)
...
model_fits <- im %>% select(rep, theta)
params_real <- params %>%
...
best_params_real %>% select(name, real_unit, real_est, real_low, real_high, real_est_years)
```
Not particularly realistic! Will need to keep optimizing. 